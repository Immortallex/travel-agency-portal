...
import { Users, Upload, Loader2, ArrowRight } from 'lucide-react';
...
export default function FamilyForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(result.error || "Database rejected the save.");
      ...
      <div className="max-w-4xl mx-auto pt-32 px-6">
        <div className="bg-white p-10 border rounded-[3rem] shadow-xl">
          <h2 className="text-3xl font-black mb-8 flex items-center gap-3 uppercase text-[#0A192F]">
            <Users className="text-blue-600"/> Family Relocation
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col">
                <label className="text-[10px] font-bold ml-2 mb-1 uppercase text-slate-400">Marital Status</label>
            ...
            <input name="dependentCount" type="number" placeholder="Number of Dependents (Children/Spouse)" className="w-full p-4 border rounded-2xl bg-slate-50" required />
            
            <textarea name="dependentDetails" placeholder="Optional: List names and ages of family members..." className="w-full p-4 border rounded-2xl h-24 bg-slate-50" />

            <textarea name="reason" placeholder="Why is your family relocating? (e.g. Children's education, Quality of life)" className="w-full p-4 border rounded-2xl h-32 bg-slate-50" required />
            ...
            <div className="border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center relative hover:bg-slate-50">
              ...
              <p className="text-sm font-bold text-slate-500 uppercase">Upload Main Applicant Passport</p>
            </div>
