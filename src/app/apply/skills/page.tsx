...
import { Briefcase, Upload, Loader2, CheckCircle2, ArrowRight } from 'lucide-react';
      ...
      <div className="max-w-4xl mx-auto pt-32 px-6">
        <div className="bg-white p-10 border rounded-[3rem] shadow-xl">
          <h2 className="text-3xl font-black flex items-center gap-3 uppercase text-[#0A192F] mb-8">
            <Briefcase className="text-blue-600"/> Skill Acquisition Pathway
          </h2>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <input name="skillType" placeholder="Primary Skill/Vocation (e.g. Nursing, Engineering)" className="p-4 border rounded-2xl bg-slate-50" required />
              <input name="yearsExp" type="number" placeholder="Years of Experience" className="p-4 border rounded-2xl bg-slate-50" required />
            </div>
            <div className="grid md:grid-cols-2 gap-6">
            ...
            <textarea name="trainingHistory" placeholder="Summary of technical or vocational training and certifications..." className="w-full p-4 border rounded-2xl h-32 bg-slate-50" required />
            ...
            <button type="submit" disabled={loading} className="w-full bg-[#0A192F] text-white py-5 rounded-2xl font-black text-xl flex justify-center items-center gap-2">
              {loading ? <Loader2 className="animate-spin" /> : "Apply & Proceed ($69.99)"} <ArrowRight />
            </button>
          </form>
        </div>
      </div>
