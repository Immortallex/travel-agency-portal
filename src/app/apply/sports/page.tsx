...
export default function SportsForm() {
  ...
  return (
    <div className="max-w-4xl mx-auto p-10 bg-white border rounded-[3rem] shadow-2xl my-12">
      <h1 className="text-2xl font-black uppercase mb-8 border-b pb-4 flex items-center gap-3"><Trophy className="text-blue-600" /> Sports Relocation</h1>
      <form onSubmit={handleSubmit} className="grid gap-6">
        ...
        <textarea name="address" placeholder="Full Residential Address" className="w-full p-4 border rounded-2xl h-24" required />
        <div className="grid md:grid-cols-2 gap-4">
          <select name="sportType" className="p-4 border rounded-2xl bg-blue-50 font-bold" required><option>Football</option><option>Basketball</option></select>
          <input name="position" type="text" placeholder="Playing Position" className="p-4 border rounded-2xl" required />
        </div>
        <div className="grid md:grid-cols-2 gap-4">
          <input name="height" type="text" placeholder="Height (e.g. 185cm)" className="p-4 border rounded-2xl" required />
          <input name="video" type="url" placeholder="Highlight Video Link (Optional)" className="p-4 border rounded-2xl" />
        </div>
