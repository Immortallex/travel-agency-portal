      ...
      <div className="max-w-4xl mx-auto pt-32 px-6">
        <div className="bg-white p-10 border rounded-[3rem] shadow-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <input name="qualification" placeholder="Highest Qualification (e.g., BSc, MSc)" className="p-4 border rounded-2xl bg-slate-50" required />
              <input name="cgpa" placeholder="CGPA or Grade" className="p-4 border rounded-2xl bg-slate-50" required />
            </div>
            ...
            <select name="country" className="w-full p-4 border rounded-2xl bg-slate-50" required>
              <option value="">Target Country</option><option>United Kingdom</option><option>Canada</option><option>Germany</option><option>USA</option>
            </select>
            <textarea name="statement" placeholder="Briefly state your academic goals..." className="w-full p-4 border rounded-2xl h-32 bg-slate-50" required />
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div className="border-2 border-dashed border-slate-200 rounded-2xl p-6 text-center relative"><input type="file" name="cv" className="absolute inset-0 opacity-0 cursor-pointer" required /><FileText className="mx-auto text-slate-300 mb-2" /><p className="text-[10px] font-bold text-slate-500 uppercase">Academic CV/Transcript</p></div>
            </div>
            <button type="submit" disabled={loading} className="w-full bg-[#0A192F] text-white py-5 rounded-2xl font-black text-