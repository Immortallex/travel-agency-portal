...
export async function POST(req: NextRequest) {
  try {
    ...
    const familyData = JSON.parse(formData.get("familyData") as string);

    const buffer = Buffer.from(await passport.arrayBuffer());
    const key = `passports/family-${Date.now()}-${passport.name}`;
    ...
    return NextResponse.json({ error: "Family submission failed" }, { status: 500 });
  }
}